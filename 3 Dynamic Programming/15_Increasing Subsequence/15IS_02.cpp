...
void solve(){
  int n; read(n);
  set<pii,greater<pii>> s = {{0,0}};
  for_(i,0,n){
    int x; read(x);
    auto itr2 = s.upper_bound({x,1});
    auto itr = make_reverse_iterator(itr2); // doubt
    itr--;      // why is this required, search google
    pii p = {x,(*itr).ss+1};
    itr++;
    while (itr!=s.rend() && (*itr).ss <= p.ss){
      s.erase(*itr);
      if (itr!=s.rend()) itr++;
    }
    if (itr!=s.rend() && (*itr).ff == p.ff) continue;
    s.insert(p);
  }
