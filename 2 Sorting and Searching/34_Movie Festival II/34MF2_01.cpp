...
int main(){
  ...
  vector<pi> v(n);
  int s,e;
  for_(i,0,n) cin>>s>>e,v[i]={e,s};
  sort(all(v));
  multiset<int,greater<int>> ms; // sorted in reverse so that lower_bound will return <= val
  for_(i,0,k) ms.insert(0);
  int ans=0;
  for_(i,0,n){
    if (*ms.rbegin()<=v[i].ss){ // needs to start from rbegin() as ms is in descending order
      ans++;
      int itv = *ms.lower_bound(v[i].ss);
      ms.erase(ms.find(itv));
      ms.insert(v[i].ff);
    }
  }
  cout<<ans;
